...
func defaultCleanup(app *cutlass7.App) {
	DestroyApp(app)
	os.RemoveAll(app.Path)
}

func UnbuiltBuildpack(depName string, copyBrats func(string) *cutlass7.App) {
	Context("Unbuilt buildpack (eg github)", func() {
		...
		BeforeEach(func() {
			bpName = bratshelper.GenBpName("unbuilt")
			app = copyBrats("")
			app.Buildpacks = []string{bpName + "_buildpack"}
			cmd := exec.Command("git", "archive", "-o", filepath.Join("/tmp", bpName+".zip"), "HEAD")
			cmd.Dir = bratshelper.Data.BpDir
			Expect(cmd.Run()).To(Succeed())
			Expect(cutlass7.CreateOrUpdateBuildpack(bpName, filepath.Join("/tmp", bpName+".zip"), "")).To(Succeed())
			Expect(os.Remove(filepath.Join("/tmp", bpName+".zip"))).To(Succeed())
		})
		AfterEach(func() {
			defaultCleanup(app)
			Expect(cutlass7.DeleteBuildpack(bpName)).To(Succeed())
		})
...
func DeployingAnAppWithAnUpdatedVersionOfTheSameBuildpack(copyBrats func(string) *cutlass7.App) {
	Describe("deploying an app with an updated version of the same buildpack", func() {
		...
		BeforeEach(func() {
			bpName = bratshelper.GenBpName("changing")
			app = copyBrats("")
			app.Buildpacks = []string{bpName + "_buildpack"}
			stackAssociationSupported, err := cutlass7.ApiGreaterThan("2.113.0")
		...
		AfterEach(func() {
			...
			if count, err := cutlass7.CountBuildpack(bpName, ""); err == nil && count > 0 {
				Expect(cutlass7.DeleteBuildpack(bpName)).To(Succeed(), "Attempted to delete buildpack %s", bpName)
			}
			// LTS errors when running `cf buildpacks`. Ignore that output.
			count, err := cutlass7.CountBuildpack(bpName, "")
			if err == nil {
		...
		It("prints useful warning message to stdout", func() {
			Expect(cutlass7.CreateOrUpdateBuildpack(bpName, Data.UncachedFile, stack)).To(Succeed())
			PushApp(app)
			...
			Expect(cutlass7.CreateOrUpdateBuildpack(bpName, newFile, stack)).To(Succeed(), "Could not create or update %s on %s", bpName, stack)
			PushApp(app)
...
func StagingWithBuildpackThatSetsEOL(depName string, copyBrats func(string) *cutlass7.App) {
	Describe("staging with "+depName+" buildpack that sets EOL on dependency", func() {
		...
		JustBeforeEach(func() {
			...
			Expect(cutlass7.CreateOrUpdateBuildpack(bpName, file, stack)).To(Succeed())
			os.Remove(file)
...
func StagingWithADepThatIsNotTheLatestConstrained(depName string, versionConstraint string, copyBrats func(string) *cutlass7.App) {
...
func StagingWithADepThatIsNotTheLatest(depName string, copyBrats func(string) *cutlass7.App) {
	StagingWithADepThatIsNotTheLatestConstrained(depName, "x", copyBrats)
}

func StagingWithCustomBuildpackWithCredentialsInDependencies(copyBrats func(string) *cutlass7.App) {
...
func DeployAppWithExecutableProfileScript(depName string, copyBrats func(string) *cutlass7.App) {
...
func DeployAnAppWithSensitiveEnvironmentVariables(copyBrats func(string) *cutlass7.App) {
...
func ForAllSupportedVersions(depName string, copyBrats func(string) *cutlass7.App, runTests func(string, *cutlass7.App)) {
...
func ForAllSupportedVersions2(depName1, depName2 string, compatible func(string, string) bool, itString string, copyBrats func(string, string) *cutlass7.App, runTests func(string, string, *cutlass7.App)) {
