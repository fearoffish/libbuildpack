...
func InitBpData(stack string, stackAssociationSupported bool) *BpData {
	...
	var err error
	Data.BpDir, err = cutlass7.FindRoot()
	...
	go func() {
		...
		cachedBuildpack, err := cutlass7.PackageUniquelyVersionedBuildpackExtra(Data.Cached, Data.BpVersion, stack, true, stackAssociationSupported)
		Expect(err).NotTo(HaveOccurred())
	...
	go func() {
		...
		uncachedBuildpack, err := cutlass7.PackageUniquelyVersionedBuildpackExtra(Data.Uncached, Data.BpVersion, stack, false, stackAssociationSupported)
		Expect(err).NotTo(HaveOccurred())
